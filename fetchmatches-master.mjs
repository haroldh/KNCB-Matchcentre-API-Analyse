...
/* ----------------------------
   Telegram (met console-logging)
-----------------------------*/
async function notifyTelegram(text) {
  if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    if (VERBOSE >= 0) console.log("ℹ️ Telegram niet geconfigureerd; skip");
    return;
  }
  try {
    if (VERBOSE >= 0) console.log("📣 Telegram →", (text || "").slice(0, 140));
    const resp = await axios.post(
      `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`,
      { chat_id: TELEGRAM_CHAT_ID, text, disable_web_page_preview: true }
    );
    if (VERBOSE >= 1)
      console.log("✅ Telegram OK (message_id:", resp?.data?.result?.message_id, ")");
  } catch (e) {
...
async function ensureInfra(sheets) {
  try { globalThis.__RUNS_NAME__ = await ensureSheet(sheets, RUNS_SHEET); } catch {}
  try { globalThis.__LOG_NAME__ = await ensureSheet(sheets, LOG_SHEET); } catch {}
  try { globalThis.__CHANGES_NAME__ = await ensureSheet(sheets, CHANGES_SHEET); } catch {}
}

function assertSheetsReady(sheets) {
  if (!sheets) throw new Error("Sheets client niet geïnitialiseerd.");
}
...
// Tekstopmaak afdwingen (id/hash-kolommen)
...
async function findHeaderMap(sheets, sheetName) {
  const resp = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!1:1`, valueRenderOption: 'UNFORMATTED_VALUE',
  });
...
function flattenObject(obj) {
  const out = {};
  for (const [k, v] of Object.entries(obj)) out[k] = v !== null && typeof v === "object" ? JSON.stringify(v) : v;
  return out;
}
function extractArray(any) {
  if (Array.isArray(any)) return any;
  if (any && typeof any === "object") {
    for (const k of ["matches", "data", "items", "rows"]) if (Array.isArray(any[k])) return any[k];
...
function pickReferrer() {
  const candidates = [
    MATCH_REFERRER_URL, GRADES_REFERRER_URL, SEASONS_REFERRER_URL, "https://matchcentre.kncb.nl/matches/",
  ].filter(Boolean);
  for (const c of candidates) {
    try { const u = new URL(c); if (/matchcentre\.kncb\.nl$/i.test(u.hostname)) return u.toString(); } catch {}
  }
...
function getGradeId(g) {
  ...
  for (const c of cands) {
    const n = Number(String(c ?? "").replace(/[^\d]+/g, "")); if (Number.isFinite(n)) return String(n);
  }
  return "";
}
...
function getByPath(obj, path) {
  try {
    return path.split(".").reduce((acc, key) => {
      ...
      const m = key.match(/(.+)\[(\d+)\]$/);
      if (m) { const k = m[1]; const i = Number(m[2]); const v = acc[k]; return Array.isArray(v) ? v[i] : undefined; }
      return acc[key];
    }, obj);
    ...
    async (u, ref, ias) => {
      try {
        const res = await fetch(u, {
          ...
          headers: { Accept: "application/json", "X-IAS-API-REQUEST": ias },
          method: "GET",
          cache: "no-store",
        });
      ...
      } catch (e) { return { __error: `Fetch failed: ${e.message}` }; }
    },
...
function getFieldValue(row, aliases) {
  for (const k of aliases) { if (k in row) return String(row[k] ?? ""); }
  return "";
}
...
async function logSummary(sheets, runId, gradeCount, matchCount, errors, ch) {
  await ensureInfra(sheets);
  const ts = nowIso();
  await appendRows(sheets, globalThis.__LOG_NAME__, [[runId, ts, SCRIPT_NAME, "main", "summary", "MASTER", "INFO", `version=${VERSION} grades=${gradeCount} matches=${matchCount} changes=${ch?.total ?? 0} (c=${ch?.created ?? 0}, u=${ch?.updated ?? 0}, d=${ch?.deleted ?? 0}) errors=${errors}`, ""]]);
...
(async () => {
  ...
  try {
    ...
    // Sheets init + run start
    if (!DISABLE_SHEETS && SPREADSHEET_ID) {
      try {
        if (VERBOSE >= 1) console.log("🔐 Init Google Sheets…");
    ...
    if (!DISABLE_SHEETS) {
      if (!SPREADSHEET_ID) throw new Error("SPREADSHEET_ID ontbreekt.");
      if (!sheets) throw new Error("Sheets client niet beschikbaar na init.");
      await ensureInfra(sheets);
    }
    ...
    if (process.env.API_URL) await fetchJsonInPage(page, process.env.API_URL, refPage, "warmup");
    ...
    if (gradesRows.length) {
      ...
      if (sheets) {
        ...
        await applyPlainTextFormatForHeaders(sheets, gradesTab, ["gradeId","grade_id","_grade_id","id","ID","Id"]);
      }
    }
    ...
    for (let i = 0; i < gradesArr.length; i++) {
      ...
      if (VERBOSE >= 0) console.log(`\n${i + 1}/${gradesArr.length}: --- 🔁 Grade ${gid} (season ${seasonInGrade || SEASON_ID || "n/a"}) ---`);
      ...
      if (rows.length) {
        ...
        if (sheets) {
          ...
          await applyPlainTextFormatForHeaders(sheets, sheetName, ["match_id","grade_id","status_id","_grade","_season","_hash","_run_id"]);
        }
    ...
    if (masterRowsRaw.length) {
      ...
      // ➜ ALTIJD een Telegram & LOG melding over changes
      ...
      try {
        ...
        if (sheets) { await ensureInfra(sheets); await appendRows(sheets, globalThis.__LOG_NAME__ || 'LOG', [[runId, nowIso(), SCRIPT_NAME, 'diff', 'changes', 'CHANGES', 'INFO', msg, '']]); }
      } catch (e) {
        console.warn('⚠️ Telegram/LOG changes melding faalde:', e?.message || e);
      }
      ...
      if (sheets) {
        ...
        await applyPlainTextFormatForHeaders(sheets, masterResolved, ["match_id","grade_id","status_id","_grade","_season","_hash","_run_id"]);
      }
    } else {
    ...
    // Summary (incl. changes) → Telegram + RUNS/LOG
    if (sheets) {
      const timestamp = nowIso();
      const gradeCount = gradesArr.length;
      const matchCount = totalMatches;
      const summaryText = JSON.stringify([
        `run started=${runId}`, `now=${timestamp}`, `script=${SCRIPT_NAME}`, `function=main`,
        `action=summary`, `table=MASTER`, `level=INFO`, `version=${VERSION}`,
        `grades=${gradeCount}`, `matches=${matchCount}`,
        `changes=${changeSummary.total}`, `created=${changeSummary.created}`,
        `updated=${changeSummary.updated}`, `deleted=${changeSummary.deleted}`,
        `errors=${errorsCount}`,
      ]);
      console.log(summaryText);
      await notifyTelegram(summaryText);
      ...
      if (SPREADSHEET_ID) {
        ...
        if (sheetsTmp) {
          ...
          await appendRows(sheetsTmp, globalThis.__LOG_NAME__ || LOG_SHEET, [
            [newRunId(), nowIso(), SCRIPT_NAME, "main", "fatal", "-", "ERROR", err.message, ""],
          ]);
        }
      }
    } catch {}
